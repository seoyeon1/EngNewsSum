...
| a | 기술 공부, 구현 | 데이터 전처리, 모델링 등 전반적인 요약 기능 구현 |
| f | 테스트 | 사용자로부터 직접입력 받아 요약문 생성 |
| k | 유지보수, 웹 페이지 개발 | Django 사용했다가 Flask로 변경 |
| k | 유지보수, 연동시키기 | Flask의 app.py에 모델을 불러와 요약문 생성 |
...
원문 텍스트에서 길이가 60 이하인 샘플의 비율 : 84%


요약된 문장에서 길이가 6 이하인 샘플의 비율 : 86%
...
전체 등장 빈도에서 희귀 단어 등장 빈도 비율 : 1.56%
  > 해당 단어들은 정수 인코딩 과정에서 배제
  

  > 몇 단어들이 제거되면서 해당 단어로 구성된 샘플이 빈 샘플이 됨
  > 빈 샘플들도 제거
